...
const Deprived = (props) =>
{
  ...
  const myHero = serverPath((heros?.find(hero => hero.type === "deprived")?.imagePath) || "");
  ...
  return (
    <div className={styles.strategicAim}>
      <SmallHero title={language.deprived} image={myHero} isRTL={isRTL}/>
      <div className={[styles.saw, "w-controller"].join(" ")}>
        <div className={styles.wrapper}>
          ...
          <div className={styles.contentWrapper}>
            <Title 
              text={language.deprived}
            />
            {depriveds.length > 0 ?
            depriveds.map(perField => (
              <div className={styles.stratigic} key={perField._id}>
                ...
                <a href={serverPath(perField.filePath)} download={"depriveds.pdf"} target="_blank" className={styles.fileLink}>{languages.downloadPlanFile}</a>
              </div>
            ))
            :
