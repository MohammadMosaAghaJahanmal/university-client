...
const ReEnrollment = (props) =>
{
  ...
  const myHero = serverPath((heros?.find(hero => hero.type === "re_enrollment")?.imagePath) || "");
  ...
  return (
    <div className={styles.strategicAim}>
      <SmallHero title={language.re_enrollment} image={myHero} isRTL={isRTL}/>
      <div className={[styles.saw, "w-controller"].join(" ")}>
        <div className={styles.wrapper}>
          ...
          <div className={styles.contentWrapper}>
            <Title 
              text={language.re_enrollment}
            />
            {reenrollments.length > 0 ?
            reenrollments.map(perField => (
              <div className={styles.stratigic} key={perField._id}>
                ...
                <a href={serverPath(perField.filePath)} download={"reenrollments.pdf"} target="_blank" className={styles.fileLink}>{languages.downloadPlanFile}</a>
              </div>
            ))
            :
