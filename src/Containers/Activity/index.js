...
const Activity = (props) =>
{
  ...
  const {heros, activities} = globalState;

  const [committees, setCommittees] = useState(activities);

  const isRTL = (language.getLanguage() === 'ps');
  const myHero = new URL(serverPath(heros?.find(hero => hero.type === "activity")?.imagePath || "")).href;
  ...
  {
    navigate(`/multipleimgs/activity/${id}`);
  }
        ...
        if(activities.length <= 0)
        {
          setIsLoading(true);
            ...
            if(objData.status === "success")
            {
              ...
              setCommittees(data);
              dispatch('setData', {type: "activities", data: data})
            }
  ...
  return (
    <div className={styles.cb}>
      <SmallHero title={language.activity} image={myHero} isRTL={isRTL} bgAnimation={true}/>
